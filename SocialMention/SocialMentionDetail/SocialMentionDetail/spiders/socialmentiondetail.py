...
from ..items import SocialmentiondetailItem
from scrapy.http import Request
from scrapy_redis.spiders import RedisSpider
from razerdateresolution import parse_12hourclock_id
import datetime


class SocialmentiondetailSpider(RedisSpider):
    name = 'socialmentiondetail'
            ...
            if response.status == 200:
                ...
                if replyNumNode:
                    ...
                    for i in range(1, int(replyNum / 3)):
                        ...
                        yield Request(url=response.url, callback=self.parse_scroll,
                                      meta={'scrollDistance': scrollDistance},
                                      dont_filter=True)

                items = html.xpath('//div[@class="stream"]/ol//li/ol//li')
                for item in items:
                    try:
                        ti = SocialmentiondetailItem()
                        ...
                        if replyTime:
                            ...
                            strdate = item.xpath('./div/div[2]/div[1]/small/a/@title')[0]
                        ti['DateofComplaint'] = parse_12hourclock_id(strdate)
